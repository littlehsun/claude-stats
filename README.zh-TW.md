...
- **24H 細項** — 15 分鐘粒度的長條圖，並顯示當天 KPI（總量、Output、Input、Session 數、最常用 Model / 最活躍專案）
- 支援 7 天 / 30 天 / 全部 篩選
...
如果你有多台電腦，可以在其中一台按 `Export` 匯出 zip，再到另一台用 `Import` 匯入。匯入時資料會直接合併進單一本機儲存區；相同 assistant 訊息會依訊息識別立即去重，所以磁碟上最後只會保留一份。
